...
## 7. 作業完了時チェック（User 向け · サーバー）

Canvas 更新 · Sprint 完了 · リンク変更を **User に返す前** に PM / 開発 AI が必ず実行する。
...
**起動手順（開発側）:** `scripts/launch/start-dev-servers.bat`  
**User 操作:** リンククリックのみ。サーバー未起動時は開発側が対応。
...
| 2026-07-05 | v1.1 — §7 作業完了時チェック（サーバー · Canvas リンク） |
