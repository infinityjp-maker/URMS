...
| **ループ** | `.cursor/resources/loop/journal.md`（**export**） | `pnpm loop:sync`（移行）· **`pnpm loop:export`** · **`pnpm ssot:sync`** | **Resource（DB）正本**（M4 · ADR-024） |

loop journal は **M1–M4 実装済**（2026-07-08）。正本は loop-entry Resource（DB）· journal.md は export のみ。

**残負債（M4 以降）**

- 監査 · リレーション（Context · task · actor）の `relates_to` — **✅ 2026-07-08**
...
| **M4（任意）** | Markdown export のみ · 正本 DB | Git diff 運用変更 | ✅ 2026-07-08 |

**M0→M4 完了（Vision Track + ADR-024）。** 移行期は `URMS_LOOP_SSOT=dual-write` で M1 動作に戻せる。
...
| 一括同期 | `POST /v1/loop/sync` · `pnpm loop:sync`（移行 · file→DB） |
| export | `POST /v1/loop/export` · `pnpm loop:export` · `ssot:sync` 末尾（M4） |
| ssot:sync | schedule + location + loop import + **loop export** |
...
| `.cursor/resources/loop/journal.md` | M4 — **export のみ**（正本 DB）· `pnpm loop:export` |
...
2. M4 で Markdown 正本をやめるか — **✅ M4 完了（DB 正本 · export）**
3. `loop-entry` → `context:current-task` への `relates_to` — **✅ 2026-07-08**（advance 時自動作成 · `relates_to` 追加）
