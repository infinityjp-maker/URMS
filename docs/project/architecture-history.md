...
| 2026-07-05 | Phase 2.7 Sprint Planning + ADR-018/019 | ADR-018, ADR-019 | PM + Architect |
| 2026-07-05 | Phase 2.8 Quality Gate + ADR-020 | ADR-020 | PM + Reviewer + Tester |
| 2026-07-05 | Phase 2.9 AI Development Governance + ADR-021 | ADR-021 | PM + Architect |
...
| Checklist | [06-phase3-master-checklist.md](../implementation/06-phase3-master-checklist.md) |

## Phase 2.7 — Sprint Planning

Phase 3 MVP を Sprint 1〜10 に分割。Version / Feature Flag 方針を ADR-018/019 で固定。

| 項目 | 内容 |
|------|------|
| Sprint Plan | [04-sprint-planning.md](../implementation/04-sprint-planning.md) |
| Dev Roadmap | [05-development-roadmap.md](../implementation/05-development-roadmap.md) |
| ADR-018 | Versioning Policy（SemVer, API /v1/, Plugin 互換） |
| ADR-019 | Feature Flag（dev/staging/prod） |
| Architecture Freeze | **ADR-006〜017 変更なし** |

### 合同レビュー（Phase 2.7 要約）

- Architect: Architecture Freeze 維持（ADR-006〜017 不変）、ADR-018/019 整合
- Reviewer: Contract 重複なし、SSOT 維持、SemVer / `/v1/` 整合
- Tester: Sprint 依存 S1→S10 妥当、DoD/レビューゲート明記
- KM: ADR-018/019 登録、Roadmap/Backlog/Glossary 更新済

## Phase 2.8 — Quality Gate

Phase 3 実装開始前の品質・レビュー・リリース基準を標準化。

| 項目 | 内容 |
|------|------|
| Quality Gate | [07-quality-gate.md](../implementation/07-quality-gate.md) |
| ADR-020 | Quality Gate 正式採用、Phase 3 開始条件更新 |
| DoR/DoD | Contract §18/§19 が唯一 SSOT（Quality Gate はゲート補助） |
| Architecture Freeze | **ADR-006〜019 のうち 006〜017 不変** |

### 合同レビュー（Phase 2.8 要約）

- Architect: Freeze 維持、ADR-020 は Contract 上位に位置せず整合
- Reviewer: Contract 重複なし（DoR/DoD 正本は Contract）、Playbook 役割分離明確
- Tester: Coverage / Test / Release Gate は Contract §13 / Sprint S9 と整合
- KM: ADR-020 登録、Roadmap/Backlog/Glossary 更新済

## Phase 2.9 — AI Development Governance

Phase 3 実装開始前の最終ガバナンス — AI と人間の共同開発運用。

| 項目 | 内容 |
|------|------|
| Governance | [08-ai-development-governance.md](../implementation/08-ai-development-governance.md) |
| ADR-021 | 三本柱（Contract / Quality Gate / Governance） |
| ADR-016 | Provider 追加は Adapter のみ — Core 変更禁止 |
| Architecture Freeze | **ADR-006〜020 のうち 006〜017 不変** |

### 合同レビュー（Phase 2.9 要約）

- Architect: Freeze 維持、ADR-021 は ADR-016 AI Manager 経路と整合
- Reviewer: Contract 重複なし、Quality Gate §9 と Governance §6 役割一致
- Tester: AI Output Validation は Quality Gate §4/§9 と整合
- KM: ADR-021 登録、Roadmap/Backlog/Glossary 更新済
