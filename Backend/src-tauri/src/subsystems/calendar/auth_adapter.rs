...
/// Start a PKCE loopback OAuth flow and store tokens + client creds to OS keyring.
pub async fn start_oauth_flow(client_id: String, client_secret: String) -> Result<Value, String> {
...
/// Return stored OAuth token JSON (if any)
pub fn get_oauth_tokens() -> Result<Value, String> {
    let kr = Entry::new("URMS", "google_oauth_token");
...
/// Sync calendar events using stored token; perform refresh if needed.
pub async fn sync_with_oauth(calendar_id: String, max_results: Option<u32>) -> Result<Vec<Value>, String> {
    let max = max_results.unwrap_or(5) as usize;

    // load token from keyring
    let kr = Entry::new("URMS", "google_oauth_token");
    let token_json = kr.get_password().map_err(|e| format!("no token stored: {}", e))?;
    let tok_val: Value = serde_json::from_str(&token_json).map_err(|e| format!("parse token json: {}", e))?;
...
/// Remove tokens and client creds from keyring
pub fn disconnect_oauth() -> Result<Value, String> {
    let kr = Entry::new("URMS", "google_oauth_token");
