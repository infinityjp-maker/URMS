...
function Do-Upload {
    ...
    try {  = Join-Path  'runner_auto_start.json.log';  = @{ status='mock_upload'; zip=(Split-Path  -Leaf); response=; attempt=; time=(Get-Date).ToString('o') };  | ConvertTo-Json -Compress | Out-File -FilePath  -Append -Encoding utf8 -ErrorAction SilentlyContinue } catch { Write-Output "WARN: failed to append to runner_auto_start.json.log: " }
    return @{ ok = True; body =  }
}
$attempt = 0
