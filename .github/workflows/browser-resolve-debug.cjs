...
(async ()=>{
  ...
  let browser = null;
  try{
    browser = await chromium.launch({ headless: true, args: ['--no-sandbox','--disable-dev-shm-usage','--host-resolver-rules=MAP tauri.localhost 127.0.0.1','--disable-features=NetworkService,NetworkServiceInProcess','--no-zygote','--single-process'] });
