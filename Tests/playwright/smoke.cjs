...
(async () => {
  ...
  try {
    ...
    if (!browser) {
      try { browser = await chromium.launch({ args: ['--no-sandbox', `--force-device-scale-factor=${process.env.DSF ? Number(process.env.DSF) : 1}`, '--host-resolver-rules=MAP tauri.localhost 127.0.0.1'] }); } catch (e) { pushInternalError(internalErrors, 'BROWSER_LAUNCH_ERROR: ' + String(e && e.message)); pushInternalError(internalErrors, 'BROWSER_LAUNCH_STACK: ' + ((e && e.stack) || '').slice(0,2000)); throw e; }
    }
    ...
    if (!page){
      const local = await chromium.launch({ args: ['--no-sandbox', `--force-device-scale-factor=${DSF}`, '--host-resolver-rules=MAP tauri.localhost 127.0.0.1'] });
