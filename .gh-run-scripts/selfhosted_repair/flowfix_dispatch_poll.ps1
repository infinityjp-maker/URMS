#!/usr/bin/env pwsh
# flowfix_dispatch_poll.ps1
# Dispatch selfheal workflow and poll for run info, download artifacts.
...
$wf | ConvertTo-Json | Out-File -FilePath (Join-Path $apiDir "workflow_selfheal_flowfix.json") -Encoding utf8
$id = $wf.id
...
for ($i = 0; $i -lt $max; $i++) {
    ...
    if ($runsRespRaw) {
        ...
        if ($runsResp.workflow_runs.Count -gt 0) {
            ...
            $obj | ConvertTo-Json | Out-File -FilePath (Join-Path $apiDir "test_workflow_results_selfheal_flowfix.json") -Encoding utf8
            $result = $obj
            break
        }
    }
...
if (-not $result) {
    ...
    $empty | ConvertTo-Json | Out-File -FilePath (Join-Path $apiDir "test_workflow_results_selfheal_flowfix.json") -Encoding utf8
    exit 3
}
...
$filesToCheck = @("selfheal_detected_issue.txt","selfheal_repair_start.txt","selfheal_repair_log.txt","selfheal_log_phase4.txt")
$foundFiles = @{}
...
$foundFiles | ConvertTo-Json | Out-File -FilePath (Join-Path $apiDir "artifacts_listing_selfheal_flowfix.json") -Encoding utf8
