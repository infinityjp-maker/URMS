...
(async () => {
  ...
  try {
    ...
    const summary = await page.$('a[href*="reports/latest"], a[href*="latest.md"], a:has-text("Latest")');
    if (!summary) throw new Error('Latest summary link not found');
    const diff = await page.$('a[href*="diffs/"], a:has-text("diff")');
    if (!diff) throw new Error('Diff link not found');
    const llm = await page.$('a[href*="llm"], button:has-text("LLM"), a:has-text("LLM")');
    ...
    try {
      ...
      // clicking may fail in file:// context; presence of diff file suffices
    }
